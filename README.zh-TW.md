...
├── output/               # 發佈的摘要 — 已提交並部署至 GitHub Pages
│   ├── index.html        # 最新摘要 HTML
...
4. 收集、過濾、排序並摘要推文
5. 將 `output/index.html` 和 `output/digest.md` 寫入 output 目錄
6. 在 build 執行時清除超過 `RAW_RETENTION_DAYS` 天的資料
7. 嘗試執行 `git add output/index.html output/digest.md && git commit -m "YYYY/M/D summary" && git push`，並印出實際 git 結果
...
每次觸發都會進行資料收集。程式內部維護一個計數器，**當天第三次執行時自動 build**（合併當日所有資料 → 寫入 `output/index.html` 與 `output/digest.md` → 嘗試 git push）。
...
每次 build 都會產生 `output/digest.md`——乾淨、省 token 的 Markdown 每日精選。這天生就是 [Hermes Agent](https://hermes-agent.nousresearch.com) 內建的 [llm-wiki](https://hermes-agent.nousresearch.com/docs/skills/) skill 的完美輸入來源。
...
```
> 把 /path/to/browser-automation/output/digest.md 加進我的 llm wiki
```
...
`src.scheduler.loop` 已經按固定間隔 build 並把 `output/digest.md` commit 到 git，你可以開一個伴隨的 Hermes session，監聽新 commit 後自動 ingest：

```
> 每次 browser-automation/output/digest.md 在 git 有變動時，自動 ingest 進我的 llm wiki
```

或是每天 build 完後對 Hermes 說一次：

```
> 把今天的 digest 從 ~/project/HERNY/browser-automation/output/digest.md ingest 進去
```
...
```
> 每天 18:30 自動抓 ~/project/HERNY/browser-automation/output/digest.md 並 ingest 進我的 llm wiki
```
...
- `output/index.html` 已提交至 git，作為 GitHub Pages 公開摘要頁面使用
- `output/digest.md` 與 `index.html` 一同提交 — AI Agent 可直接存取 `https://你的網域/digest.md`，以純 Markdown 格式讀取摘要，節省 token
