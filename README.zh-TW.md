...
- **自動清除** — 每次執行後刪除超過 7 天的原始捕捉目錄與摘要封存檔
- **Git 自動推送** — 寫入 `index.html` 和 `digest.md` 後自動執行 `git add`、`git commit`（訊息格式：`2026/4/13 summary`）、`git push`
...
│   ├── raw/              # 每次執行的原始捕捉（保留 7 天）
│   └── summaries/        # 每次執行的 HTML + JSON 封存（保留 7 天）
...
2. 瀏覽至 `X_HOME_URL`
3. 捲動 `SCROLL_COUNT` 次（預設 120），每次暫停 `SCROLL_PAUSE_SECONDS` 秒
4. 收集、過濾、排序並摘要推文
5. 將 `index.html` 和 `digest.md` 寫入專案根目錄
6. 清除超過 `RAW_RETENTION_DAYS` 天的資料
7. 執行 `git add index.html digest.md && git commit -m "YYYY/M/D summary" && git push`
...
| `CDP_REMOTE_DEBUGGING_PORT` | `9333` | Chrome CDP 埠號 |
...
每次觸發都會進行資料收集。程式內部維護一個計數器，**當天第三次執行時自動 build**（合併當日所有資料 → 寫入 `index.html` → git push），不需要額外的指令旗標。
...
# 第 3 次 — 晚上收集 + 自動 build + git push（摘要當晚發布）
